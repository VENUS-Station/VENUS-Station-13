	...
	if(tooltips)
		if(!QDELETED(usr) && !QDELETED(src) && usr?.client?.prefs.enable_tips)
			...
			if(length(tooltip_data))
				...
				usr.client.tip_timer = addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(openToolTip), usr, src, params, name, examine_data), timedelay, TIMER_STOPPABLE)//timer takes delay in deciseconds, but the pref is in milliseconds. multiplying by 0.01 converts it.
