...
/obj/item/portallight/attack(mob/living/carbon/human/M, mob/living/carbon/human/user)
	...
	if(user_message)
		if(portal_target && (portal_target?.client?.prefs.toggles & VERB_CONSENT || !portal_target.ckey))
			...
			if(M.can_penetrating_genital_cum() && M.handle_post_sex(user_lust_amt, portalunderwear.targetting, portal_target, TRUE, TRUE)) //SPLURT edit
				switch(target)
					if(CUM_TARGET_PENIS)
						switch(portalunderwear.targetting)
							if(CUM_TARGET_PENIS)
			...
			if(portal_target.handle_post_sex(target_lust_amt, portalunderwear.targetting, M, TRUE, TRUE)) //SPLURT edit
				switch(portalunderwear.targetting)
					if(CUM_TARGET_VAGINA)
						switch(target)
							if(CUM_TARGET_PENIS, CUM_TARGET_VAGINA, CUM_TARGET_ANUS, CUM_TARGET_MOUTH)
