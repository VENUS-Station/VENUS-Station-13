/client/looc(msg as text)
	...
	if(isnull(vetted_check))
		...
		to_chat(usr, span_warning("You must be vetted in order to use LOOC. Please get vetted through the appropriate channels.<br>Ahelp or ask staff in the discord if this is an error."))
		return
