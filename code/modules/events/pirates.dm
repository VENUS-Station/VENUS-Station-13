...
/proc/send_pirate_threat()
	...
	threat_msg.answer_callback = CALLBACK(GLOBAL_PROC, PROC_REF(pirates_answered), threat_msg, payoff, ship_name, initial_send_time, response_max_time, ship_template)
	addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(spawn_pirates), threat_msg, ship_template, FALSE), response_max_time)
